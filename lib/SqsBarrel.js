...
sqsbarrel.extendedNewDivision = async function ( division ) {
	return 'ok'
}

sqsbarrel.extendedRemoveEntity = async function ( division, context, name) {
	return 'ok'
}

sqsbarrel.extendedNewEntity = async function ( division, context, name) {
	var self = this
...
sqsbarrel.innerProcessSqs = function ( comm ) {
	...
	if ( !comm.response ) {
		...
		return self.appease( realComm, comm.err ? new Error(comm.err) : null, responses )
	}
}

sqsbarrel.intoxicateAnswer = async function ( comm, err, responseComms ) {
	let self = this
	...
	let packet = JSON.stringify( { id: comm.id, comm: comm, nodeSeqNo: self.nodeSeqNo, err: err ? err.message || err.toString() : null, response: true, responseComms: responseComms || [] } )
...
sqsbarrel.intoxicateMessage = async function ( comm ) {
	let self = this
