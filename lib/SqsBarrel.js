...
sqsbarrel.appease = function ( comm, err, responseComms ) {
	...
	try {
		self.queues[ comm.sourceDivision ].publish( entityName + SEPARATOR + nodeNO, packet, 'utf8')
	} catch (err) {
...
sqsbarrel.intoxicate = function ( comm ) {
	...
	let nodeNO = comm.nodeSeqNo || self.nodeNO( comm )
	try {
		self.queues[ comm.division ].publish( entityName + SEPARATOR + nodeNO, packet, 'utf8')
	} catch (err) {
		self.logger.harconlog( err )
	}
}
