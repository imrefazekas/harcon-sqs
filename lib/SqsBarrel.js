...
sqsbarrel.sendToQueue = function ( division, message, callback ) {
	this.sqs.sendMessage({ MessageBody: message, QueueUrl: this.queues[ division.hx() ], DelaySeconds: 0 }, function (err, data) {
...
sqsbarrel.newDivision = function ( division, callback ) {
	var self = this
	if ( self.queues[ division.hy() ] ) return Proback.quicker( 'ok', callback )
	return new Promise( (resolve, reject) => {
...
sqsbarrel.appease = function ( comm, err, responseComms ) {
	...
	if ( !self.queues[ comm.division.hy() ] )
