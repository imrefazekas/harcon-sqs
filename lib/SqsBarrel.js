...
sqsbarrel.sendToQueue = function ( division, message, callback ) {
	this.sqs.sendMessage({ MessageBody: message, QueueUrl: this.queues[ hy( division ) ], DelaySeconds: 0 }, function (err, data) {
...
sqsbarrel.createQueue = function ( division, listening, callback ) {
	...
	let _division = division.hx()
	if (self.queues[ _division ]) return callback()
	...
	self.sqs.createQueue( { QueueName: division }, function (err, data) {
		...
		self.queues[ _division ] = data.QueueUrl

		if (listening)
			self.sqs.receiveMessage({ QueueUrl: self.queues[ _division ] }, function (err, data) {
				console.log('<<<<<<<<<', err, data)
				...
				data.Messages.forEach( (message) => {
					try {
						...
						if ( !interested )
							self.sqs.changeMessageVisibility({ QueueUrl: self.queues[ _division ], ReceiptHandle: message.ReceiptHandle, VisibilityTimeout: 0 }, function (err, data) { if (err) self.logger.harconlog(err) } )
						else
							self.sqs.deleteMessage({ QueueUrl: self.queues[ _division ], ReceiptHandle: message.ReceiptHandle }, (err) => { if (err) self.logger.harconlog(err) } )
...
sqsbarrel.newDivision = function ( division, callback ) {
	var self = this
	if ( self.queues[ hy( division ) ] ) return Proback.quicker( 'ok', callback )
	return new Promise( (resolve, reject) => {
...
sqsbarrel.appease = function ( comm, err, responseComms ) {
	...
	if ( !self.queues[ hy( comm.division ) ] )
