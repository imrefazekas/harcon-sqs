...
sqsbarrel.nodeNO = function ( comm ) {
...
sqsbarrel.createQueue = function ( division, listening, callback ) {
	...
	if (self.ins[division]) return callback()
	...
	self.sqs.createQueue( { QueueName: division }, function (err, data) {
		...
		self.ins[division] = data

		if (listening)
			self.sqs.receiveMessage({ QueueUrl: self.ins[division] }, function (err, data) {
				...
				data.Messages.forEach( (message) => {
					try {
						...
						if ( !interested )
							self.sqs.changeMessageVisibility({ QueueUrl: self.ins[division], ReceiptHandle: message.ReceiptHandle, VisibilityTimeout: 0 }, function (err, data) { if (err) self.logger.harconlog(err) } )
						else
							self.sqs.deleteMessage({ QueueUrl: self.ins[division], ReceiptHandle: message.ReceiptHandle }, (err) => { self.logger.harconlog(err) } )
...
sqsbarrel.extendedClose = function ( callback ) {
	...
	return new Promise( (resolve, reject) => {
		...
		let fns = []
		Object.keys(self.ins).forEach( function (domain) {
			fns.push( function (cb) { self.sqs.purgeQueue( { QueueUrl: self.ins[domain] }, () => { cb() } ) } )
		} )
		Object.keys(self.outs).forEach( function (division) {
			fns.push( function (cb) { self.sqs.purgeQueue( { QueueUrl: self.ins[division] }, () => { cb() } ) } )
		} )
		async.series( fns, () => {
